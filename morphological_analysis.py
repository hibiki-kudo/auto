...
def save_csv(user_name, words):
    df = pd.DataFrame(words[:500:], columns=["単語", "回数", "割合"])
    df.to_csv(f"./csvファイル/{user_name}_analysis.csv")
...
if __name__ == "__main__":
    ...
    df.to_csv(f"./csvファイル/{file_name[:-19]}_analysis.csv")
